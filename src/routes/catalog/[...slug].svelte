  ...
  function filterData(filtersData) {
    ...
    for (let filter of sortedFiltersData) {
      if (filter.current && filter.name !== "price") {
        ...
        currentFlowerList = currentFlowerList.filter((flower) => {
          if (filter.current.from <= flower.price && filter.current.to >= flower.price) {
            return true
          }
          return false
        })
...
<div class="flex mb-16 w-full catalog-page">
  <div
    class="h-full catalog-content rounded {$orderStore.orderList.length
      ? 'has-aside'
      : ''}"
  >
    ...
    <div class="item-list rounded-b pb-8">
      <div class="px-44 lg:px-34 xl:px-80 pt-34 border-gray-300 border-r border-l">
