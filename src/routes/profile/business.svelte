...
<script>
    ...
    onMount(() => {
        headers = [ActiveOrderHeader, OrderHistoryHeader, DeliveryAddressesHeader, Reporting, Employees, Requisites];
...
<div>
    <div class="flex justify-between">
        ...
        border-r border-l border-gray-300">
                ...
                <Button className="-mt-6"  size="full" textClass="text-base"  status="{businessUser == false ? 'disabled-main' : 'active-main'}" on:click={toggleAddAddress}>
                    Добавить адрес доставки
                </Button>

                <section>
                    <h2 class="text-2xl font-bold text-main" data-scrollspy bind:this={Reporting}>Отчётность</h2>
                    <p>История заказов по всем сотрудникам и акты, доступные для скачивания и печати.</p>
                    {#if businessOrder.report}
                    <div class="reportButtons flex">
                            {#each reportButtons as reportButton, i}
                                <div class="reportButton">
                                    <OutlineRadio text={reportButton} label="reportButton{i}" bind:value={reportButton} bind:group={selectedReportButton} name={selectedReportButton}/>
                                </div>
                            {/each}
                    </div>
                        ...
                        {:else}
                           <p class="mb-34 mt-12">Сделайте свой первый заказ, и все документы по нему тут же появятся здесь.</p>
                            <Button size="lg">Перейти в каталог</Button>
                    {/if}

                </section>
                <section>
                    <h2 class="text-2xl font-bold text-main" data-scrollspy bind:this={Employees}>Список сотрудников</h2>
                    <p class="mb-30">
                ...
                <section >
                    <h2 class="text-2xl font-bold text-main" data-scrollspy bind:this={Requisites} >Реквизиты</h2>
                    <p>Работает автоподстановка — начните вводить название юр. лица или ИП</p>
                    <div class="businessRequisites">
                        <BusinessRequisites bind:businessUser={businessUser}/>
                    </div>
                </section>
            </div>
        </div>
