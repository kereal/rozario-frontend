   ...
   import { currTabStore, cartFormStore } from "../../stores/cart.js";
    ...
    onMount(async() => {
        ...
        let dynamicMask = IMask(
                document.getElementById('email-mask-settings'), {
                            mask: /^\S*@?\S*$/
                });
        let dateMask = IMask(
                document.getElementById('date_mask_settings'), {
                    mask: Date,
                    lazy: false
                });
        })
    ...
    $: if(values) {
         if(values.name != userData.name || values.tel != userData.tel || values.email != userData.email || values.BirthDay != userData.BirthDay || values.selectedPhoto != userData.selectedPhoto
                 || values.sms != userData.sms || values.push != userData.push || values.email != userData.email) {
            changed = true
        }
    }

    function beforeUnload(e) {
        if(values.name != userData.name || values.tel != userData.tel || values.email != userData.email || values.BirthDay != userData.BirthDay ||
                values.selectedPhoto != userData.selectedPhoto || values.sms != userData.sms || values.push != userData.push || values.email != userData.email) {
...
<style>
    ...
    .settings__content {
        background: linear-gradient(180deg, #FFFFFF 0%, rgba(255, 255, 255, 0) 100%), linear-gradient(0deg, #F2F2F2, #F2F2F2);
        width: 100%;
        ...
        <div class="settings__content">
            <div>
                ...
                <div class="settings__card relative">
                    <div class="flex">
                    <button class="absolute settings__button"
                    ...
                    > <span>...</span>
                        ...
                        <CustomDropdown
                                ...
                                position="CENTER">
                            <div class="bg-white rounded pl-24 pr-24 items-center dropDownDelete flex" on:click={showModalDelete} on:mouseleave={showDelete}>
                                <svg class="mr-8" width='16px' height='17px' xmlns="http://www.w3.org/2000/svg" viewBox="0 0 32 32">
                                    <path class="" d="M29.1 5.6h-6.3C22.5 2.4 20.7 0 18.4 0h-4.8c-2.3 0-4.1 2.4-4.4 5.6H2.9c-.6 0-1.1.5-1.1 1.1s.5 1.1 1.1 1.1h1.4v20.5C4.3 30.3 6 32 8 32h16c2 0 3.7-1.7 3.7-3.7V7.8h1.4c.6 0 1.1-.5 1.1-1.1s-.4-1.1-1.1-1.1zM13.6 2.3h4.8c.9 0 1.9 1.4 2.1 3.3h-9.1c.3-1.9 1.3-3.3 2.2-3.3zm11.8 26c0 .7-.7 1.4-1.4 1.4H8c-.7 0-1.4-.7-1.4-1.4V7.8h18.9v20.5z"/>
                                    <path class="" d="M13 24c.6 0 1.1-.5 1.1-1.1v-9.2c0-.6-.5-1.1-1.1-1.1s-1.1.5-1.1 1.1v9.2c-.1.6.5 1.1 1.1 1.1zM19 24c.6 0 1.1-.5 1.1-1.1v-9.2c0-.6-.5-1.1-1.1-1.1-.6 0-1.1.5-1.1 1.1v9.2c0 .6.5 1.1 1.1 1.1z"/>
                                </svg>
                        ...
                        <div class="settings__photo flex">
                                ...
                                <div class="miniPhoto  w-full h-full">
                                    <div class="w-full h-full">
                                        <img src='{values.selectedPhoto}' alt='avatar' class="w-full h-full">
                                    </div>
                                </div>
                            {/if}
                            <div class="settings__photo_change flex w-full absolute content__perspective" id="settings__photo_animate"
                        ...
                        <div class="settings__inputs">
                           ...
                           <div class="settings__input">
                            ...
                            <div class="settings__input">
                                ...
                                <input type="text" id="date_mask_settings"  bind:value={values.BirthDay}>
                            </div>
                            <div class="settings__input">
                            ...
                            <div class="settings__input">
                                <span>E-mail</span>
                                <input type="email" id="email-mask-settings"  bind:value={values.email}>
                            </div>
                           </div>
                        </div>
                </div>
                    <div>
                        ...
                        <p class="actions__text">
                            Подпипишитесь на уведомления и получите промокод на скидку.<br/>Еженедельно получайте полезные статьи, а также информацию о скидках и акциях
                        </p>
                        <ul>
                            <li class="mb-16">
                                <CustomCheckbox style="font-size: 14px;" value="Push"  className="text-main"  id="Push" label="Push-уведомления" bind:check={values.push}/>
                            </li>
                            <li class="mb-16">
                                <CustomCheckbox  style="font-size: 14px;" value="sms" className="text-main" id="sms" label="SMS"  bind:check = {values.sms}/>
                            </li>
                            <li>
                                <CustomCheckbox style="font-size: 14px;" value="email" className="text-main"  id="email" label="Электронная почта"   bind:check={values.email}/>
                            </li>
                        </ul>
                    </div>
                    <div class="buttons flex ml-100">
                        <Button textClass="text-base" size="md" status="{changed == false ? 'disabled-main' : 'active-main'}" on:click={()=>{
                        changed = false;
                        userData = values;
                        ...
                        {#if changed == true}
                            <Button textClass="text-base" size="md" status="active-secondary" className="ml-24" on:click={
                                    ()=>{
...
{#if ModalPhoto}
    <ModalChangePhoto
            ...
            bind:fullImage={fullImage}
            bind:miniBlob={miniBlob}/>
{/if}
{#if ModalChangeData}
    <ConfirmationModal
