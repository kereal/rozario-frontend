...
<script>
    ...
    export let favoriteBouquets = [
        {img: 'favorite-card', title: 'Настоящая Любовь', time: 'Доставим в течение <span class="text-main">120</span> мин', price: {sale: 3456, full: 3456}, rating: '4,5', size: {text: 'Размеры', width: 50, height: 50}, description: 'Описание состава', like: true},
        {img: 'favorite-card', title: 'Настоящая Любовь', time: 'Доставим в течение <span class="text-main">120</span> мин', price: {sale: 3456, full: 3456}, rating: '4,5', size: {text: 'Размеры', width: 50, height: 50}, description: 'Описание состава', like: true},
        {img: 'favorite-card', title: 'Настоящая Любовь', time: 'Доставим в течение <span class="text-main">120</span> мин', price: {sale: 3456, full: 3456}, rating: '4,5', size: {text: 'Размеры', width: 50, height: 50}, description: 'Описание состава', like: true},
        {img: 'favorite-card', title: 'Настоящая Любовь', time: 'Доставим в течение <span class="text-main">120</span> мин', price: {sale: 3456, full: 3456}, rating: '4,5', size: {text: 'Размеры', width: 50, height: 50}, description: 'Описание состава', like: true},
        {img: 'favorite-card', title: 'Настоящая Любовь', time: 'Доставим в течение <span class="text-main">120</span> мин', price: {sale: 3456, full: 3456}, rating: '4,5', size: {text: 'Размеры', width: 50, height: 50}, description: 'Описание состава', like: true},
...
<div>
    <div class="flex justify-between">
        <div class="w-full">
            ...
            <div bind:this={containerElement} class="favorites wrapper box-content flex flex-col border-b border-r border-l border-gray-300">
                <h2 data-scrollspy bind:this={FavoriteBouquetsHeader} class="favorites__heading font-bold text-main flex items-center">Избранные букеты {#if favoriteBouquets.length > 0}<span class="block text-gray-700 text-lg font-normal">({favoriteBouquets.length})</span>{/if}</h2>
                {#if favoriteBouquets.length > 0}
                    <section class="favorites__items flex flex-wrap">
                        {#each favoriteBouquets as bouquet, i}
                            <FavoriteCard recommendation={bouquet} onHeartClick={(value) => toggleBouquetDeleteModal(value, i)}/>
                        {/each}
                    </section>
                {:else}
                    <section class="favorites__empty">
                        <p class="favorites__text">Понравился букет? Добавь его в избранное, чтобы вернуться к нему в любой момент </p>
                        <a href="/" class="favorites__button">Перейти в каталог</a>
                    </section>
                {/if}
                <h2 data-scrollspy bind:this={FavoriteGiftsHeader} class="favorites__heading favorites__heading--2 font-bold text-main flex items-center">Избранные подарки {#if favoriteGifts.length > 0}<span class="block text-gray-700 text-lg font-normal ml-12">({favoriteGifts.length})</span>{/if}</h2>
                {#if favoriteGifts.length > 0}
                    <section class="favorites__items flex flex-wrap">
                        {#each favoriteGifts as bouquet, i}
                            <FavoriteCard recommendation={bouquet} onHeartClick={(value) => toggleGiftDeleteModal(value, i)}/>
                        {/each}
                    </section>
                {:else}
                    <section class="favorites__empty">
                        <p class="favorites__text">Понравился подарок? Добавь его в избранное, чтобы вернуться к нему в любой момент </p>
                        <a href="/" class="favorites__button">Перейти в каталог</a>
                    </section>
                {/if}
                <h2 class="favorites__heading favorites__heading--2 font-bold text-main flex items-center">Недавно просмотренные</h2>
                <section class="favorites__items flex flex-wrap">
                    {#each recentlyViewed as bouquet, i}
                        <FavoriteCard recommendation={bouquet} onHeartClick={() => toggleRecentlyViewedHeart(i)}/>
                    {/each}
                </section>
            </div>
        </div>
...
<DeleteFavoriteModal showModal={showBouquetDeleteModal} cardDeleted={cardDeleted} toggleModal={toggleBouquetDeleteModal} deleteCard={deleteBouquet}/>
<DeleteFavoriteModal showModal={showGiftDeleteModal} cardDeleted={cardDeleted} toggleModal={toggleGiftDeleteModal} deleteCard={deleteGift}/>
