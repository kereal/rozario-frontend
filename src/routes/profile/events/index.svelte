...
<div>
    <div class="flex justify-between">
        ...
        border-r border-l border-gray-300 events-card-wrapper">
                ...
                <Calendar events={eventsComming} allEvents={$eventsStore} {today} year={new Date().getFullYear()} month={new Date().getMonth()}/>
                <h2
                        data-scrollspy
                ...
                <ul class="-mr-40 flex flex-col lg:flex-row flex-wrap">
                    {#each eventsSections as section, i}
                        <span class="text-main font-semibold title-month">{formatSectionTitle(section.title)}</span>
                        {#each section.list as event, i}
                            <li class="w-full lg:w-1 pr-40 cards-event">
                                <EventCard id={event.id} index={i} event={event}/>
                            </li>
                        {/each}
                    {/each}
                </ul>
                ...
                {/if}
                <div class="buttonAdd">
                <Button className="w-full"  size="full" textClass="text-base" on:click={toggleAddEvent}>
                    Добавить событие
                </Button>
                </div>
            </div>
