...
<div class="w-full bg-gray-800 main-page mb-10 border-b border-gray-300">
  <Hero name={data.cityMeta.name} locativus={data.cityMeta.locativus} />
  {#if promoBlock}
    <div class="border-r border-l border-gray-300 px-20 bg-white">
      <RozarioGameCard />
    </div>
  {/if}
  <CatalogNavbar initCategories={data.categories} page="index" />
  <SectionContainer>
    <div class="my-slider-wrapper flex flex-col mx-auto">
      ...
      {#if data.flowersList}
        <Carousel
          ...
          itemsInRow={3}
        >
          {#each data.flowersList as flower, i}
            <div
  ...
  <div id="collections" class="block" style="width:0; height:0; margin:0; padding:0;" />
  ...
  <SectionContainer>
    ...
    <div class="justify-between flex items-stretch flex-wrap -mb-40 -mr-40">
      {#if data.categories}
        {#each data.categories as collection}
    ...
    <div data-value="CUSTOMERSCONTAINER" bind:this={customersCarouselContainer}>
      {#if data.testimonials}
        <Carousel
          ...
          totalitems={data.testimonials?.length}
        >
          {#each data.testimonials as testimonial, i}
            <div
              style={`width: calc(100% / ${4});`}
              class="customers-carousel-item carousel-item {i < 4
    ...
    <div data-value="BLOGPOSTSCONTAINER" bind:this={blogpostsCarouselContainer}>
      {#if data.blogposts}
        <Carousel
          ...
          totalitems={data.blogposts?.length}
        >
          {#each data.blogposts as blogpost, i}
            <div
              style={`width: calc(100% / ${3});`}
              class="blog-post-carousel-item carousel-item {i < 3
                ? 'carousel-item-active'
                : ''}"
            >
  ...
  <SectionContainer>
    <h2 class="text-main font-bold text-2xl mb-24 leading-tight">
      Улыбки наших получателей
    </h2>
    <p class="text-gray-700 mb-48 text-lg">
