    ...
    export let orders = [
        {
            ...
            comment: 'Сам не спец в цветах, но жене очень понравился оригинальный весенний букет,' +
            ...
            products: [
                ...
                },
                {
                    ...
                    postcard: 'Тебе желаю море счастья, Улыбок, солнца и тепла.' +
        ...
        <div class="content">
            <section class="ActiveOrder">
                <h2
                        ...
                        class="sectionName">Активные заказы</h2>
                        <div class="buttons">
                            <AutoWidthButtons value={selectedActiveButton} bind:group={selectedActiveButton} selectedText="Еще" list={reportButtons} buttonElement=""/>
                        </div>
                {#if activeOrderShow != []}
                    {#each activeOrderShow as order}
                        <div class="activeOrder">
                            <ActiveBusinessOrder bind:order={order}/>
                        </div>
                    {/each}
                {/if}
                {#if activeOrderShow == []}
            ...
            <section>
                ...
                class="sectionName">История заказов</h2>
                <p class="historyText">История заказов и акты, доступные для скачивания и печати.</p>
                <BusinessHistoryOrder bind:orders={orderHistory}/>
            </section>
            <section>
                <h2
                        data-scrollspy
                        bind:this={PretensionHeader}
                class="sectionName">Претензии</h2>
                <div class="pretensionButtons">
                    {#each pretensionsStatus as status, i}
                        <div class="mr-14 last:mr-0" >
                            <OutlineRadio text={status} label="status{i}" bind:value={status} bind:group={selcetedPretension} name={selcetedPretension} />
                        </div>
                    {/each}
                </div>
