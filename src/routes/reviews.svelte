...
<script>
    ...
    let loadMoreReviews = function() {
        loading = true;
        let reviewsToLoad = cityMeta.reviews.slice(startIndex, startIndex + step);
        console.log(reviewsToLoad);
        startIndex = startIndex + reviewsToLoad.length;
        addReviewsToColumns(reviewsToLoad);
        if (startIndex >= cityMeta.reviews.length) {
            hasMoreReviews = false;
        }
    };
    let isLeftColumnHigher = function() {
        return document.getElementById('leftColumnReviews').offsetHeight
                >
...
<style>
    ...
    @media (max-width: 1300px) {
        .header, .reviews-body, .reviews-body :global(button .text-base) {
            font-size: 15px;
        }
...
<div id="reviews-page"
    ...
    <div class="header image-box relative">
        ...
        <p>Здесь собраны все отзывы, оставленные покупателями и счастливыми получателями после доставки.</p>
    </div>
