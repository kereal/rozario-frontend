...
<style>
    ...
    @media (max-width: 1300px) {
        .header, .smiles-body, .smiles-body :global(button .text-base) {
            font-size: 15px;
        }
...
<div id="smiles-page"
    ...
    <div class="smiles-body">
        {#if cityMeta}
            {#each shownSmiles as smile, i}
                <RecipientSmile {smile} smile_i="{'smile' + i}" openSmileModal="{openSmileModal}" />
            {/each}
        {/if}
    </div>
    {#if hasMoreSmiles}
    ...
    {#if smilesModalOpened}
        <RecipientSmileModal smiles="{smiles}" smile_i="{smile_i}" closeModal="{closeSmileModal}" />
    {/if}
</div>
