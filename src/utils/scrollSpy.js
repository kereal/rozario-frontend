...
class ScrollSpy {
  constructor(element, headers) {
    ...
    this._element.addEventListener("scroll", this.throttledProcess)
    this.refresh()
    this._process()
  }
    ...
    targets.forEach((element) => {
      if (element) {
        ...
        if (elementBCR.width || elementBCR.height) {
          this._offsets.push(
            elementBCR.top + document.body.scrollTop - this._headerOffset + offsetBase
          )
        }
      }
    })
  ...
  destroy() {
    this._element.removeEventListener("scroll", this.throttledProcess)
  }
