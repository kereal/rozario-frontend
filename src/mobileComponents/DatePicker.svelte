<script>
    ...
    function selectDay(e) {
        ...
        if (
                today.getTime() >
                setDate(currCalendarDate, parseInt(e.target.innerText)).getTime()
        ) {
            return;
        }
    ...
    function selectYear() {
        ...
        if (isBefore(currCalendarDate, today)) {
            ...
            currCalendarDate = new Date(today.getTime());
        } else if (currCalendarDate.getFullYear() === today.getFullYear() &&
...
<div class="wrapper h-full">
    <div class="h-32 w-full flex justify-between items-center mb-4">
        <IconButton
                on:click={prevMonth}
                className="fill-current {prevButtonStatus === 'disabled' ? 'text-gray-1100' : 'text-infolight hover:text-info'}"
                iconSize="33"
                icon={ArrowLeftSmall}
                status="only-icon" />
                            ...
                            <ul class="yearlist">
                                {#each getYearRange() as year}
                                    <li>
                                        <button
                                                on:click={selectYear}
                                                data-fullyear={`${year}:${currCalendarDate.getFullYear()}:${currCalendarDate.getFullYear() === year}`}
                                                class="{currCalendarDate.getFullYear() === year ? 'selected font-semibold' : ''}
    ...
    <div class="flex flex-wrap" bind:this={datePickerElement}>
        ...
        {#if prevEnd.getDay() > 0}
            {#each createFirstRange(prevEnd.getDate() - prevEnd.getDay() + 1, prevEnd.getDate()) as day}
                <button on:click={selectPrevMonthDay} class="{setDate(sub(currCalendarDate, {months: 1}), parseInt(day.getDate())).getTime() < today.getTime() ? 'cursor-default' : ''} day text-gray-700">
                    {day.getDate()}
                </button>
            {/each}
        {/if}
        {#each createRange(1, getDaysInMonth(currCalendarDate)) as day}
            <button
                    on:click={selectDay}
                    class="{getDayNumberStyle(currDate, day)} day">
                {day.getDate()}
            </button>
        {/each}
        {#if nextStart.getDay() === 0}
            <button
                    on:click={selectNextMonthDay}
                    class="day hover:bg-gray-200 text-gray-700">
                1
            </button>
        {:else}
