<script>
  ...
  function nextDate() {
    daysCounter++
    currentDate = format(add(new Date(), { days: daysCounter }), "dd MMMM yyyy", {
      locale: ru
    }).split(" ")
  }
  function prevDate() {
...
>
  <div class="mb-16 pb-8 w-full border-b border-gray-500 flex flex-col items-center">
  ...
  <button on:click={timeClarify} class="text-main mx-auto text-center mb-8 font-semibold">
    Уточнить у получателя
  </button>
  <button on:click={timeAsap} class="text-main mx-auto text-center mb-8 font-semibold">
    Как можно быстрее
  </button>
