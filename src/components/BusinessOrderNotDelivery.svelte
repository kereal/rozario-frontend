<script>
    ...
    let reasons = [
        ...
        'Указан некорректный адрес доставки',
        'Указан некорректный телефон получателя',
        'Не указан телефон получателя',
            'Получатель отказывается принять заказ',
        'Другая причина']
...
border: 1px solid var(--gray-300);"
              on:close={closeModal}>
    <h2>Укажите, почему заказ не был доставлен:</h2>
    ...
    {/if}
    <div class="buttons">
        <Button size="lg" status="{selectedReason == 'Выберите причину'? 'disabled-main':'active-secondary'}" className="mr-24">Завершить выполнение заказа</Button>
    </div>
</ModalSurface>