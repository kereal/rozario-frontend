...
<div class="fav-card wrapper flex flex-col rounded-lg">
    <div class="fav-card__image-placeholder rounded-t-lg bg-white relative">
        <picture>
            ...
            <img class="main-image rounded-t-lg object-contain absolute inset-0 h-full w-full" data-src="{recommendation.img}.png" alt={recommendation.title}/>
        </picture>
        <div on:mouseenter={toggleDeal} on:mouseleave={toggleDeal} bind:this={dealElement} class="deal-box">
            <div class="text-black">%</div>
        </div>
        {#if dealVisible}
            <CustomDropdown css="width: 300px; max-height: 300px;" bind:dropdownVisible={dealVisible} relativeElement={dealElement} position="LEFT">
                <section class="bg-white px-32 py-24 rounded-lg">
                    <h3 class="text-main font-semibold text-lg pb-16 border-b border-gray-300 mb-16">Избранное</h3>
                    <p class="text-gray-700 text-sm truncate">Добавлено в избранное</p>
                </section>
            </CustomDropdown>
        {/if}
        <div on:mouseenter={() => {infoVisible = true;}} on:mouseleave={() => {infoVisible = false;}} bind:this={infoElement} class="info-box">
            <svg class="w-full h-full" version="1.1" xmlns="http://www.w3.org/2000/svg" width="36.643px" height="36.644px" viewBox="0 0 36.643 36.644">
                <g>
                    <g>
                        <polyline fill="none" stroke="#333333" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" stroke-miterlimit="10" points=" 16.596,14.711 18.459,14.711 15.161,27.212 17.947,27.212 "/>
        ...
        {#if infoVisible}
            <CustomDropdown css="width: 300px; max-height: 300px;" bind:dropdownVisible={infoVisible} relativeElement={infoElement} position="RIGHT">
                <section class="bg-white px-32 py-24 rounded-lg">
    ...
    <div class="fav-card__meta bg-white rounded-b-lg">
            ...
            <div class="flex justify-between items-center h-24">
                ...
                {#if recommendation.like}
                    <div on:click={() => onHeartClick(true)} class="fav-card__heart-wrap flex">
                        <svg class="fav-card__heart cursor-pointer" width="24" height="21" viewBox="0 0 24 21" fill="none" xmlns="http://www.w3.org/2000/svg">
                            <path d="M12 21C12.0247 20.9887 24 15.5319 24 7.97984C24 -0.534669 15.662 -1.89944 11.9888 2.2927C8.30435 -1.8611 0 -0.487152 0 7.99487C0 15.4363 12 21 12 21Z" fill="#E71F75"/>
                        </svg>
                    </div>
                {:else}
                    <div on:click={() => onHeartClick(false)} class="fav-card__heart-wrap flex">
                        <svg class="fav-card__heart cursor-pointer" width="24" height="21" viewBox="0 0 24 21" fill="none" xmlns="http://www.w3.org/2000/svg">
                            <path d="M12 21C12.0247 20.9887 24 15.5319 24 7.97984C24 -0.534669 15.662 -1.89944 11.9888 2.2927C8.30435 -1.8611 0 -0.487152 0 7.99487C0 15.4363 12 21 12 21Z" fill="#E5E5E5"/>
                        </svg>
                    </div>
                {/if}
            </div>
        ...
        <div class="fav-card__meta-second text-main flex justify-between items-center">
            {#if recommendation.price.sale}
                <div>
                    <span class="fav-card__price-new font-bold mr-12 text-main whitespace-no-wrap">
                        {recommendation.price.sale} ₽
                    </span>
                    <span class="fav-card__price-old font-normal line-through text-gray-500 whitespace-no-wrap">
                        {recommendation.price.full} ₽
                    </span>
                </div>
            {:else}
                <span class="fav-card__price-new font-semibold text-main whitespace-no-wrap">
                    {recommendation.price.full} ₽
                </span>
            {/if}
