...
<Portal>
    ...
    <p class="whitespace-no-wrap mainText">
        Укажите корректные цены на составляющие. Для отстутствующих составляющих предложите замену.
    </p>
    <div class="products">
        ...
        <div class="product">
            ...
            <div class="product__body">
                ...
                {#each product.composition as composition}
                    <CompositionProductElement bind:composition={composition}/>
                {/each}
            </div>
        </div>
    {/each}
...
<div class="footer flex justify-between items-center">
    <div class="buttons flex">
        <Button size="lg" status="{changed == true ? 'active-main' : 'disabled-main'}" className="mr-24"
                on:click={()=>{
                visible = false;
                }}>Сохранить изменения</Button>
        <Button size="lg" status="active-secondary" on:click={()=>{order = cloneDeep(changeOrder);}}>Отменить</Button>
    </div>
    <div class="price">
