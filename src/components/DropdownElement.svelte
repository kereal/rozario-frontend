...
<div class="flex items-end leading-snug">
  <button
    ...
    >
    <span bind:this={textElement} class="{currentFilter ? 'font-semibold' : ''}  mr-8 whitespace-no-wrap leading-snug" >{currentFilter ? currentFilter.text : text}</span>
    {#if !currentFilter}
        <svg
...
{#if currentFilter}
  <IconButton on:click={removeFilter} className="pb-2" size="xxs" status="only-icon" icon={Close} iconSize="9" iconStyle="stroke-current text-main" />
{/if}
</div>
{#if currentFilter}
  {#if dropdownVisible}
