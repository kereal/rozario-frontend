...
<section
  class="delete-modal-wrap {showModal ? 'delete-modal-wrap--show' : ''}"
  on:click={() => toggleModalWrap(false)}
>
  <div
    class="delete-modal {showModal ? 'delete-modal--show' : ''}"
  ...
  >
    ...
    {:else}
      <h3 class="delete-modal__heading text-main">
        Вы действительно хотите удалить этот товар из избранного?
      </h3>
      <div class="delete-modal__buttons">
        <button
          ...
          on:click={(e) => {
            e.stopPropagation()
            toggleModalWrap(false)
          }}>Нет</button
        >
        <button
          class="delete-modal__button delete-modal__button--gray"
          on:click={(e) => {
            e.stopPropagation()
            doDeleteToggle()
          }}>Да</button
