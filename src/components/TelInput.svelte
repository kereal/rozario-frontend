<script>
      ...
      case "ArrowUp":
        if (selectedCountry.element.previousElementSibling) {
          ...
          selectedCountry.element =
            selectedCountry.element.previousElementSibling;
          setTimeout(() => selectedCountry.element.focus(), 0);
          selectedCountry.element.tabIndex = 0;
  ...
  onMount(() => {
    ...
    selectedCountry.element = document.querySelector(
      '.tel-input ul li[tabindex="0"]'
    );
...
<svelte:window
  on:click={e => {
    if ((countryList && countryList.contains(e.target)) || countrySelectButton.contains(e.target)) {
      return;
    }
    visible = false;
  }} />
...
<div
  ...
  class="tel-input flex relative items-center rounded {className}" style="height:{height}px;">
  <button
    type="button"
