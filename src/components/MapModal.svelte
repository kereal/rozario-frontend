<script>
  ...
  function handleAutoCompleteItemClick(e) {
    ...
    if (closest.dataset.number.length > 0) {
      autoCompleteInputValue = closest.dataset.street + ", " + closest.dataset.number
      closeAllLists()
    } else {
      ...
      data = initialData.filter((item) => {
        ...
        return (
          item.street.toLocaleLowerCase().substring(0, autoCompleteInputValue.length) ===
...
<Portal>
  <div transition:fade bind:this={mapModal} on:click={closeViaOverlay} class="overlay">
    <div
    ...
    >
      <section class="mb-18">
        <h2 class="leading-tight text-main text-3xl mb-32">Укажите адрес доставки</h2>
          ...
          >
            ...
            {:else}
              <button
                ...
                font-bold text-lg"
              >
                <div class="flex justify-center items-center text-gray-700 h-32">ОК</div>
              </button>
            {/if}
          </div>
        </div>
