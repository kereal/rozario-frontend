<script>
  ...
  function formatDate(date) {
    ...
    switch (date.status) {
      ...
      case "exact":
        return fnsFormat(new Date(order.date.value), "EEEEEE, d MMMM, в HH:mm", {
          locale: ru
        })
      case "notime":
        return date.notime
...
<div class="content flex flex-col h-full w-full">
  ...
  <div class="text-gray-600 block text-one-five lg:hidden leading-tight mb-18">
    Мы получили данные и оформляем ваш букет, служба доставки и другая полезная информация
    о данном этапе.
  </div>
  <div class="flex flex-col lg:flex-row h-full">
    ...
    <div class="content-right flex flex-col h-full justify-between lg:justify-start">
