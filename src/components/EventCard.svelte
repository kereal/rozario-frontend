...
<div class="relative h-full wrapper {isComing && index !== 0 && 'border-top-clr border-t'} {isComing ? 'pb-18': `rounded border border-gray-300 px-24 py-14 ${daysTillToday <= 7 ? 'bg-white' : 'bg-gray-200'}`} ">
    ...
    {#if isComing}
        <div class="margin-left">
            <Button className="{!!event.description ? 'mt-16-media' : 'mt-without-decr' } gift-button block button--md " withMarginLeft="{true}" on:click={sendGift} size="md" status="active-main">
                Подобрать подарок
            </Button>
        </div>
    {/if}
