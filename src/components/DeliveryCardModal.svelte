<script>
    ...
    let once = true;
    let tab = 'other';
    const deliveryPerionds = ['Доставка к точному времени с 21:00 до 08:00',
    ...
    let deliveryParams = {
        'Доставка к точному времени с 21:00 до 08:00': {
            cost:150, time: 21, until:'08', period: false, toogle:'PeriodOne'
        },
        'Доставка интервал 2 часа с 21:00 до 08:00': {
            cost:250, time: 21, until:'08', period: 2, toogle:'PeriodTwo'
        },
        'Доставка к точному времени с 08:00 до 21:00': {
            cost:350, time: '08', until:'21', period: false, toogle:'PeriodThree'
        },
        'Доставка интервал 2 часа с 08:00 до 21:00': {
            cost:450, time: '08', until:'21', period: 2, toogle:'PeriodFour'
        },
        'Уточнить время у получателя' : {
            cost:550, time: '21'
        },
    ...
    ymaps.geocode(city).then(function (res) {
        ...
        myMap.events.add('click', function (e) {
            ...
            fetch('http://geocode-maps.yandex.ru/1.x/?apikey=239f3c15-dce2-4008-8563-b9f9ca399b0b&format=json&results=1&geocode='+ deliveryСoordinates.reverse())
                    .then(response => response.json())
                    .then(commits => deliveryAdress = commits.response.GeoObjectCollection.featureMember[0].GeoObject.name)
        });
 fetch('http://geocode-maps.yandex.ru/1.x/?apikey=239f3c15-dce2-4008-8563-b9f9ca399b0b&format=json&results=1&geocode=' + city)
                .then(response => response.json())
                .then((commits) => {shopAdress = commits.response.GeoObjectCollection.featureMember[0].GeoObject.Point.pos.split(' ').reverse()
...
let deliveryСoordinates
    ...
    function getPlaceDelivery(commits) {
        myMap.geoObjects.remove(PlacemarkShop)
        deliveryСoordinates = commits.response.GeoObjectCollection.featureMember[0].GeoObject.Point.pos.split(' ').reverse()
        PlacemarkShop = new ymaps.Placemark(deliveryСoordinates);
        myMap.geoObjects.add(PlacemarkShop);
        myMap.setCenter(deliveryСoordinates)
   }
    ...
    <div transition:fade on:click={closeViaOverlay} class="overlay">
        <div bind:this={modalElement}
            ...
            {#if value=='Удалённая доставка'}
                ...
                <div>
                    <div class="flex">
                        <input bind:value={deliveryAdress} placeholder="Укажите адрес" class="input__city text-main" on:blur={
                                            fetch('http://geocode-maps.yandex.ru/1.x/?apikey=239f3c15-dce2-4008-8563-b9f9ca399b0b&format=json&results=1&geocode=' + deliveryAdress)
                                            .then(response => response.json())
                                            .then(commits => getPlaceDelivery(commits))}>

                            <Button status="{deliveryAdress == '' ? 'disabled-main' : 'active-main'}" size="lg" on:click={Calculate}>
                                Рассчитать
                            </Button>

                    </div>
                    <div class="map" id="minimap" style="width:632px; height:417px;"></div>
                </div>
            </div>
             <div class:postcardAfterMapVis>
                 <h2 class="deliverycard__name">Стоимость доставки</h2>
                 {#if onCity}
                     <p class="delivery__text mb-8">Длина маршрута <span class="delivery__text_param">{city} —  {deliveryAdress}, 1 = {long} км </span></p>
                     <p class="delivery__text mb-34">Расчёт стоимости удалённой доставки  производится по тарифу <span class="delivery__text_param">1км = {delivery_price} ₽</span></p>
                     {:else}
