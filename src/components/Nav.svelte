<script>
  ...
  async function selectCity(e) {
    ...
    const r = await fakeFetch(
      `/api/cities?city=${$mainStore.address.city.name}`
    );
    cityInput = "";
...
<header
  ...
  <div
    ...
    <div class="flex items-center ml-32">
      ...
      <div
        class="bg-accent rounded-full h-32 text-main flex items-center mr-16">
        <div
    ...
    {#if helpDrawerVisible}
      <HelpDrawer
            buttonRef={helpDrawerButton}
            on:close={toggleHelpDrawer}  />
    {/if}
  </div>
  {#if mapModalVisible}
