  ...
  function calculatePosition() {
    ...
    if (relativeElementRect.y - 300 - 160 > 0 && direction === "TOP") {
      ...
      if (position === "CENTER") {
        ...
        popupArrowBottom.style.left = relativeElement.offsetWidth / 2 + 7 + "px"
      } else if (position === "RIGHT") {
        popupElement.style.right = `-12px`

        popupArrowBottom.style.right = relativeElement.offsetWidth / 2 - 3 + "px"
      }
    } else {
...
<div on:mouseover={calculatePosition} on:mouseout={leave} class="rounded relative outer">
