<script>
    ...
    <div class="calendar-wrap border border-gray-300 border-container ">
        ...
        <div class="upcoming-events flex flex-col">
            ...
            {#if events.length > 0}
                <div on:scroll={onScroll} class="mt-24 flex h-full custom-scrollbar overflow-auto border-top-clr border-t">
                    <ul class="flex flex-row flex-wrap mr-20">
                        {#each events as event, i}
                            <li class="w-full lg:w-1">
                                <EventCard id={event.id} index={i} event={event} isComing={true}/>
                            </li>
                        {/each}
                    </ul>
                </div>
            {:else}
                <p class="flex mt-18 pt-18 border-top-clr border-t font-p mr-18">У вас пока не запланировано никаких событий.
                    Добавьте в календарь важную дату, и мы заранее напомним о поздравлении!</p>
            {/if}
            <div class="flex gradient-shadow {listScrollTopMax - listScrollTop > 30 ?'' : 'animate'}"/>
        </div>
    </div>
<style>
    .border-container {
