...
<div
  ...
  <div class="text-left">
    {#each text as line, i}
      <div class="text-left text-white leading-mid text-sm" style="{styleText}"><span class="whitespace-no-wrap" style="{i == 1? styleForSecond : ''}">{line}</span></div>
    {/each}
  </div>
  {#if iconPos === 'right'}
  ...
  {#if iconAfter}
    <div class="w-10 ml-14">
      <img class="w-full h-full" width="24px" height="24px" src="{iconAfter}.svg" alt={iconAfter} style="{iconAfterCss}"/>
    </div>
  {/if}
</div>
{#if dropdownVisible}
