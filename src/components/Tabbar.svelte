<script>
  ...
  function checkSizePlus() {
    if (count != 0) {
      buttonDropMoreCategories = document.getElementById(
        "buttonDropMoreCategories"
      ).offsetWidth
      maxWidth = document.getElementById("navTabbar").offsetWidth
      buttonsWidth = document.getElementById("ulTabbar").offsetWidth
  ...
  $: if (currentMenuIndex) {
    if (
      currentMenuIndex <
      document.getElementById("ulTabbar").childNodes.length - count
    ) {
      selectedItem = "Еще"
    } else {
    ...
    setTimeout(() => {
      ...
      if (
        document.getElementById("navTabbar").offsetWidth <
        document.getElementById("ulTabbar").offsetWidth + dropdownWidth + 88
      ) {
        count = count + 1
        buttons[buttonsLength - count].style.display = "none"
...
>
  ...
  >
    {#each menulist as menu, i}
      <li
        class="horrizontalItem h-full flex items-center border-b-4 {currentMenuIndex ===
        menu.index
          ? 'border-accent'
          : 'border-transparent'}"
      >
  ...
  {#if count != 0}
    <div class="inline-flex items-center h-full forDrop">
      <button
        id="buttonDropMoreCategories"
        bind:this={moreDropdownButton}
        on:click={showMoreNav}
        class="h-full flex items-center border-b-4 {selectedItem != 'Еще'
          ? 'border-accent'
          : 'border-transparent'} buttonDrop"
      >
        <div
          class="flex items-center {moreDropdownVisible
            ? 'bg-gray-300'
            : ''} px-16 leading-12 whitespace-nowrap rounded-full text-main"
        >
          <span
            class="items-center mr-8 h-full whitespace-nowrap text-main block {selectedItem !=
            'Еще'
              ? 'font-semibold'
              : 'font-normal'}">{selectedItem}</span
          >
          <img
