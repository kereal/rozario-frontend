<script>
  ...
  function onComplete({ detail: imask }) {
    console.log("complete tel input");
    value = imask.value;
    dispatch("completed");
  }
  function onAccept({ detail: imask }) {
    console.log("accept tel input");
    value = imask.value;
    dispatch("accept");
  }
  let buttonStyle;
...
<div class="{className} flex items-center rounded wrapper">
  <div
    class="text-main flex items-center rounded-l pr-4 pl-12 text-one-five conteiner">
    +7
  </div>
  <input
    ...
    class:focusElement type="tel"
    on:focus={()=>{focusElement = true}}  on:blur={()=>{focusElement = false}}/>
