...
export let requisites = {
    ...
    $: if(requisites) {
            if(values.name != '' && values.INN != '' && values.KPP != '' && values.legaladdress != '' && values.bankName != '' && values.email != '') {
                filled = true
            }
    }
...
<div class="BusinessRequisites">
    ...
    <div class="BusinessRequisites__left">
        ...
        <div class="BusinessRequisites__input">
            <h3>КПП</h3>
            <input type="text" onkeyup="this.value = this.value.replace(/[^\d]/g,'');" bind:value={values.KPP}>
        </div>
    </div>
    ...
    {#if !businessUser}
    <div class="flex">
        <Button size="lg" className="mt-30" status="{filled == false ? 'disabled-main' : 'active-main'}" on:click={()=>{
        businessUser = true
        requisites = values
        values = cloneDeep(requisites);
        }}><span class="whitespace-no-wrap">Стать корпоративным клиентом</span></Button>
        <a href="/business">
            <Button size="lg" className="ml-24 mt-30">Отменить</Button>
        </a>
    </div>
    {:else}
        <Button size="lg" className="mt-30" status="{changed == false ? 'disabled-main' : 'active-main'}" on:click={()=>{requisites=values}}>Сохранить</Button>
    {/if}
</div>