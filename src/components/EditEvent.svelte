...
<ScrollableModalSurface
    ...
    <div class="block">
        ...
        {/if}
        {#if currentEditSection === section.DESCRIPTION}
            <label for="description" class="label-text margin-bottom-8">Описание (необязательно)</label>
            <TextArea
                    id="description"
    ...
    <div class="block">
        ...
        <div class="flex flex-row justify-between mt-12">
            <CustomCheckbox id="1-day" label="1 день" check={event.checkboxes[1]} on:click={() => handleRemember(1)}/>
            <CustomCheckbox id="3-day" label="3 дня" check={event.checkboxes[3]} on:click={() => handleRemember(3)}/>
            <CustomCheckbox id="7-day" label="7 дней" check={event.checkboxes[7]} on:click={() => handleRemember(7)}/>
        </div>
    </div>
