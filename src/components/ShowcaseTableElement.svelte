...
<div class="element" style={product.selected ? "" : "background:white;"}>
  <div
    class="element__header flex items-center justify-between {openElement == false
      ? 'MyBorder'
      : ''}"
  >
    <div class="flex items-center">
      <div class="mr-24">
        <CustomCheckbox id={product.name} label="" bind:check={product.selected} />
      </div>
      <div
      ...
      >
        ...
        <div>
          <svg
            ...
            fill="none"
            class="mt-2"
            class:rotated={openElement}
            on:click={() => {
              openElement = !openElement
            }}
          >
            <path
              d="M13 1L6.99465 7L1 1"
              stroke="#330033"
    ...
    <div
      on:click={() => {
        ChangeShowcaseComplitation = !ChangeShowcaseComplitation
      }}
    >
      <svg
        xmlns="http://www.w3.org/2000/svg"
          ...
          <div class="element__item flex justify-beetween">
            <div class="element__name-color flex elent-text">
              {#if color.style != undefined}
                <div class="element__colorCube" style="background:{color.style}" />
              {/if}
...
{#if ChangeShowcaseComplitation}
  <ModalChangeShowcaseComplitation
    bind:visible={ChangeShowcaseComplitation}
    bind:product
    {onePrice}
  />
{/if}
