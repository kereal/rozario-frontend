<script>
  ...
  if (!testimonial.hasOwnProperty("image")) {
    ...
    const ratingTotal = !!Object.keys(testimonial.rating || {}).length
      ? Object.entries(testimonial.rating).reduce((summ, r) => summ + r[1], 0) / Object.keys(testimonial.rating).length
      : -1
    if (ratingTotal == 5 || ratingTotal == -1) { testimonial.image = 'reviews/good.svg' }
    else if (ratingTotal >= 3 && ratingTotal < 5) { testimonial.image = 'reviews/meh.svg' }
    else { testimonial.image = 'reviews/bad.svg' }
  }
...
{#if !version}
  <div class="wrapper flex flex-col bg-cardgray px-16 pt-16 pb-24 text-center
  items-center">
    <div
  ...
  <div class="clients-review">
    <div class="testimonial-image rounded-full border-gray-500 w-full relative
    mx-auto">
      <picture>
        <source
                data-srcset="{testimonial.image}, {testimonial.image}-2x.webp 2x" />
