  ...
  <div class="flex flex-col justify-between">
    ...
    <div class="flex flex-col-reverse lg:flex-row">
      <div class="content-left">
        <div
          class="text-gray-600 text-one-five leading-mid font-semibold mb-18">
        ...
        <div class="flex">
          ...
          <div class="flex flex-col">
            <IconButton
              ...
              className="mb-8 lg:mb-16 {nextButtonStatus === 'enabled' ? 'arrow-button-enabled' : 'arrow-button-disabled'}"
              on:click={handleNext}
              status="only-icon"
              size="sm" />
            <IconButton
              className="{prevButtonStatus === 'enabled' ? 'arrow-button-enabled' : 'arrow-button-disabled'}"
              icon={ArrowLeftBig}
              iconStyle="fill-current"
              iconSize="35"
              on:click={handlePrev}
      ...
      <div class="content-right">
        <div class="info-text">
          В магазине получили данные по вашему заказу и начали его оформление,
          как только он будет готов, мы вышлем вам его фотографию.
        </div>
      </div>
    </div>
  </div>
