<script>
  ...
  function handleTimeRangeFromComplete({ detail: mask }) {
    ...
    const [dateVal, _] = date.value.split("T");
    date.value = dateVal + "T" + mask.value;
    date.time.range.to = fnsFormat(
      add(new Date(date.value), { hours: 2 }),
      "HH:mm"
    );
  }
...
<ScrollableModalSurface
          ...
          <fieldset class="info w-full flex flex-col">
            <div class="mb-8 flex items-center">
              ...
              <button
                ...
                <LetterI
                  iconSize="8"
                  classNames="text-infolight fill-current" />
              </button>
            ...
            <div class="mb-8 flex items-center">
              ...
              {#if timeGroup === 'range'}
                <div class="ml-14 flex items-center">
                  <label
                    class="text-sm text-gray-600 mr-6"
                    for="editmodal-time-from">
                    с
                  </label>
                  <TextInput
            ...
            <div class="flex items-center">
              <div class="mr-12">
                <CustomRadioButton
                  ...
                  label={recipientGroup === 'myself' ? 'Уточнить по телефону' : 'Уточнить у получателя'}
                  bind:group={timeGroup}
                  value="notime" />
              </div>
              <button
              ...
              {#if noTimeHelpVisible}
                <CustomDropdown
                  ...
                  <div class="bg-white p-18 text-gray-600 text-sm leading-mid">
                    {#if recipientGroup === 'myself'}
                      Мы заранее позвоним вам и уточним, где и в какое время вам
                      будет удобно получить заказ
                    {:else}
                      Мы сами заранее позвоним получателю и уточним, где и в
                      какое время ему будет удобно получить подарок
                    {/if}
                  </div>
                </CustomDropdown>
              {/if}
  ...
  {#if recipientGroup === 'myself'}
    <div
      class={currentEditSection === section.RECIPIENT_NAME ? 'mb-24' : 'mb-16'}>
  ...
  {#if recipientGroup === 'anotherPerson'}
    <div
      class={currentEditSection === section.RECIPIENT_PHONE ? 'mb-24' : 'mb-16'}>
        ...
        {#if noAddressHelpVisible}
          <CustomDropdown
            ...
            <div class="w-full bg-white p-18 text-gray-600 text-sm leading-mid">
              ...
              {:else}
                Мы сами заранее позвоним получателю и уточним, где и в какое
