...
<script>
  ...
  function selectFlower() {
    ...
    const index = $orderStore.orderList.findIndex((item) => item.flower.id === flower.id)
    if (index !== -1) {
...
<a
  href={desktop ? "/product/cardMobile" + flower.id : "/product/" + flower.id}
  class="rounded"
>
  <div
    ...
    >
      <div class="mb-12 flex relative">
        <div class="w-full flex flex-wrap">
          <h3 class="overflow-y-hidden inline-block mb-4 mr-4 pr-24 leading-tight title">
            {flower.title}
          </h3>
          <div class="text-gray-700 font-semibold text-sm inline-flex items-center">
        ...
        <span class="price leading-tight font-semibold whitespace-nowrap">
          {flower.price}&nbsp;₽
        </span>
      </div>
          ...
          >
            <section class="bg-white px-32 py-24 rounded-lg">
              ...
              <p class="text-gray-700 text-sm">
                ...
                потратить их сразу или накопить. Что бы посмотреть сколько купонов на
                вашем счете, перейдите в раздел «Мои купоны»
              </p>
            </section>
          </CustomDropdown>
        {/if}
