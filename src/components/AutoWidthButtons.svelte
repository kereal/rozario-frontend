<script>
    ...
    function resizeMinus() {
        if(moreButton.length == 0) {
            if (document.getElementById(group+'Line').offsetWidth > document.getElementById(group).offsetWidth) {
                count = count + 1;
        ...
        if(moreButton.length != 0){
            if (document.getElementById(group+'Line').offsetWidth + 110 > document.getElementById(group).offsetWidth) {
                count = count + 1;
                buttons[buttonsLength - count].style.display = "none";
                moreButton.push(list[buttonsLength - count])
                resizeMinus()
    ...
    function resizePlus() {
        if (document.getElementById(group+'Line').offsetWidth < document.getElementById(group).offsetWidth + 150) {
        ...
        {#each list as item, i}
            <div class="OutLineRadio {button}">
                <input
                        ...
                        id={item.id}
                />
                <label class="cursor-pointer flex items-center rounded leading-snug border border-gray-500 hover:text-main hover:border-gray-700 bg-white" for={item.id}
                       on:click={() => {
                                ...
                                <div class="custom-scrollbar overflow-y-auto rounded py-16 bg-white z-10 dropDown">
                                    <ul class="w-full">
                                        {#each moreButton as item, i}
                                            <li class="hover:bg-gray-200 w-full cursor-pointer text-main dropLi" >
                                                <input
                                                        bind:group={group}
                                                        type="radio"
                                                        class="visuallyhidden"
                                                ...
                                                <label
                                                        ...
                                                        if (document.getElementById(group+'Line').offsetWidth + 150 > document.getElementById(group).offsetWidth) {
                                                        count = count + 1;
                                                        buttons[buttonsLength - count].style.display = "none";
                                                        moreButton.push(list[buttonsLength - count])
                                                        resizeMinus()
