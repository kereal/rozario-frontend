...
{#if status == "enabled"}
  ...
  >
    ...
    {:else}
      <Icon name={icon} style="width: 8px; height:13px;" className="text-gray-1100" />
    {/if}
  </button>
{:else if status == "disabled"}
  ...
  >
    {#if typeof icon === "function"}
      <svelte:component this={icon} classNames="fill-current text-gray-1100" {iconSize} />
    {:else}
      <Icon name={icon} style="width: 8px; height:13px;" className="text-gray-1100" />
    {/if}
  </button>
{:else if status == "action"}
  ...
  >
    {#if typeof icon === "function"}
      <svelte:component this={icon} iconSize="24" classNames="fill-current text-main" />
    {:else}
      <Icon name={icon} style="width: 8px; height:13px;" className="text-gray-300" />
    {/if}
  </button>
...
{:else if status == "close"}
  <button type="button" data-testid={testid} on:click>
    <Close iconSize={16} classNames="stroke-current text-gray-1100 hover:text-gray-700" />
  </button>
{/if}
